...
int kenny_emul_vertex_count=0;
extern bool k_pred_reg[4];
//int  global_partial_cull_cnt=0;
...
void GPUEmulator::processRegisterWrite(GPURegister gpuReg, u32bit gpuSubReg, GPURegData gpuData)
{
    ...
    switch (gpuReg)
    {
        ...
        case GPU_TEXTURE_NON_NORMALIZED:
            ...
            GPU_DEBUG(
		      printf("Write GPU_TEXTURE_NON_NORMALIZED[%d] = %c", gpuSubReg, gpuData.booleanVal ? "T" : "F");
            )
...
void GPUEmulator::clearColorBuffer()
{
	...
	//kenny implements the FBO code lookup
...
/*#else
    ...
    //kenny add presentation for 1/19 show PNG picture.
    /*
    char exe_string[100];
    sprintf(exe_string,"pkill feh");
    system(exe_string);
    //sprintf(exe_string,"feh -x -A --cycle-once %s.png &",filename);
    sprintf(exe_string,"feh -x -A --cycle-once %s.png &",filename);
    system(exe_string);
    */
    //end kenny add
    ...
    if (!skipBatchMode)
    {
        ...
        for(u32bit instance = 0; instance < state.streamInstances; instance++)
        {
            GLOBALPROFILER_ENTERREGION("emulateStreamer", "", "emulateStreamer")
   	      kenny_emul_vertex_count=0;
            emulateStreamer(instance);
	    //cout << "kenny_emul_vertex_count: " <<  kenny_emul_vertex_count << endl;
            GLOBALPROFILER_EXITREGION()
...
/*if (state.indexedMode)
{
    ...
    shadedIndices.push_back(currentIndex);
	    ...
	    //kenny dump pre-shaded-vertex
	    //printf("%f, %f, %f, %f\n", attributes[0][0], attributes[0][1], attributes[0][2], attributes[0][3]);
...
void GPUEmulator::emulateVertexShader(ShadedVertex *vertex)
{
    ...
    while (!programEnd)
    {
	  ...
	  /* kenny disable noisy vertex debugging
        GPU_DEBUG(
...
void GPUEmulator::emulatePrimitiveAssembly()
{
    ...
    while (remainingVertices >= requiredVertices)
    {
		...
		// kenny dump before resterization
		// sometime will lose some vertex dunno why?
		/*
				kenny_attrib = vertex1->getAttributes();
				cout << kenny_attrib[0][0] << ","
					<< kenny_attrib[0][1] << ","
					<< kenny_attrib[0][2] << ","
					<< kenny_attrib[0][3] << endl;
				kenny_attrib = vertex2->getAttributes();
				cout << kenny_attrib[0][0] << ","
					<< kenny_attrib[0][1] << ","
					<< kenny_attrib[0][2] << ","
					<< kenny_attrib[0][3] << endl;
				kenny_attrib = vertex3->getAttributes();
				cout << kenny_attrib[0][0] << ","
					<< kenny_attrib[0][1] << ","
					<< kenny_attrib[0][2] << ","
					<< kenny_attrib[0][3] << endl;
		*/
		...
		// kenny add to dump vertex attribute (use with dummy vertex shader program to 
		// dump the vertex list without loss of vertex.)
		/*
				kenny_attrib = vertex1->getAttributes();
				cout << kenny_attrib[0][0] << ","
					<< kenny_attrib[0][1] << ","
...
void GPUEmulator::emulateRasterization(ShadedVertex *vertex1, ShadedVertex *vertex2, ShadedVertex *vertex3)
{
    ...
    else
    {
        ...
        if (!dropTriangle)
        {
			...
			//kenny dump ShadedFragments right before FS
			//cout << __FILE__<< " " << __LINE__ << endl;		      
			//fragmentCount++;
			  /*
			  for (int i = 0; i < 4; i++)
			    {
			      cout << __FILE__ << __LINE__ << endl;
			      QuadFloat* kenny_temp;
			      kenny_temp = quad[i]->getAttributes();
			      cout << kenny_temp[0];
			      cout << endl;
			    }
			  */
			  ...
			  //kenny maybe can implement discard() here
...
void GPUEmulator::emulateFragmentShading(ShadedFragment **quad)
{
    ...
    //kenny add to store quad state
    //ShadedFragment original_quad[4];
    //ShadedFragment correct_quad[4];

    //memcpy(original_quad, quad, sizeof(quad));
    ...
    while (!programEnd)
    {
        ...
        for(p = 0; p < STAMP_FRAGMENTS; p++)
        {
		  ...
		  /* kenny disable noisy fragment debugging
                GPU_DEBUG(
                    char shInstrDisasm[256];
                    shDecInstr->getShaderInstruction()->disassemble(shInstrDisasm);
		  ...
		  */
                ...
                //  Execute instruction.
		  printfspam("ghfghwerewr");
                shEmu->execShaderInstruction(shDecInstr);
		
		//if(k_pred_reg[p]==false) // Steve creating the discard bitvector
		    //STEVETEMP |=1<<p;
	...
	//kenny: some flag to identify discard and set threadKill(p) [find the method to set]
    }
    
    /*
      {  FILE* STEVEOUTFILE=fopen ("STEVEOUTFILE","a");
         fprintf(STEVEOUTFILE,"%c\n",(STEVETEMP<10)?STEVETEMP+'0':STEVETEMP-10+'A');
         fclose(STEVEOUTFILE);
      }
    */
    //kenny: to stop the simulator if the r50 and r11 are different
    { 
      if (r50nr11diff)
	{ printf("I stopped!\n");
	  cin.ignore();
...
void GPUEmulator::emulateTextureUnit(TextureAccess *texAccess)
{
    ...
    //kenny display the texture unit used
    //cout << "TexUnit: " << texAccess->textUnit << endl;
...
/*if (batchCounter == 622) {
    for(u32bit b = 0; b < (STAMP_FRAGMENTS); b++) {
	    ...
	    //kenny dump states
	    /*
	    cout << "colorBlend: " << state.colorBlend[rt] << endl;
	    cout << "multiSampling: " << state.multiSampling << endl;
	    cout << "validateionMode: " << validationMode << endl;
	    */

	    //kenny mod (now disabled)
	    // validationMode = true;

	    //int cnt=0; //kenny counting local quad cull
...
/*if (batchCounter == 622) {
    for(u32bit b = 0; b < (STAMP_FRAGMENTS); b++) {
	    ...
	    //Steve:(!state.colorBlend[rt]) && (!(((rt == 0) && state.logicalOperation))) && (!state.multiSampling)
	    ...
	    /*
	    if (cnt > 0 && cnt < 4)
	      global_partial_cull_cnt++; // kenny

	    ofstream oFile;
	    oFile.open("global_partial_cull");
	    oFile << global_partial_cull_cnt << endl;
	    oFile.close();
	    */
