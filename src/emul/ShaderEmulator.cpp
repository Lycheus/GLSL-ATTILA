...
void ShaderEmulator::updatedumpfile(ShaderEmulator &shEmul)
{ 
  //FILE *f =fopen("~/thatfile","w");
  int i;
  for (i=1;i<64;i++)
  { 
    QuadFloat* reg_val1 = (QuadFloat *) &temporaryBank[(0 * numTemporaryRegs + i) * UNIFIED_TEMP_REG_SIZE];
    const float* reg_val = reg_val1->getVector();
    //fprintf(f,"r%d:%f,%f,%f,%f\n",i,reg_val[0],reg_val[1],reg_val[2],reg_val[3]);
    printf("r%d:%f,%f,%f,%f\n",i,reg_val[0],reg_val[1],reg_val[2],reg_val[3]);
  }
  //fclose(f);
}

int ShaderEmulator::Diff50and11(ShaderEmulator &shEmul)
{cntttt++;
  //if (cnt%10000==0) printf("I got up to %d\n",cnt);
    QuadFloat* reg_val1 = (QuadFloat *) &temporaryBank[(0 * numTemporaryRegs + 50) * UNIFIED_TEMP_REG_SIZE];
    QuadFloat* reg_val2 = (QuadFloat *) &temporaryBank[(0 * numTemporaryRegs + 11) * UNIFIED_TEMP_REG_SIZE];
    const float* X = reg_val1->getVector();
    const float* Y = reg_val2->getVector();
    if (X[0]!=Y[0]) return 1;
    if (X[1]!=Y[1]) return 1;
    if (X[2]!=Y[2]) return 1;
    return 0;
...
void ShaderEmulator::dumpifIwanna(ShaderEmulator &shEmul)
{ 
  if (Diff50and11(shEmul))
    {
      ShaderEmulator::updatedumpfile(shEmul);
      r50nr11diff = 1;
    }
  else
    r50nr11diff = 0;
...
//kenny DEBUG
//#define KENNY_DBG
//kenny virtual memory
//ExternalMemory EM_firstEntry;

#define MAX_MEMORY_ADDRESS 4096

f32bit ExternalMemory[STAMP_FRAGMENTS][MAX_MEMORY_ADDRESS][5];
//kenny vertice count
extern int vertice_count;

//kenny add R15 global variable to hack value stored in register 15 for texture unit index created by NSYSU GLSL
u32bit R15_0_uint;
int kenny_dump_flag;

//kenny add global to keep tracking the prediction register for KIL instruction
bool k_pred_reg[4];
...
void ShaderEmulator::writeTextureAccess(u32bit id, QuadFloat *sample, u32bit *threads, bool removeInstruction)
{
    ...
    for(u32bit i = 0; i < stampFragments; i++)
    {
	    ...
	    //kenny dump tex instruction result
	    /*
	    if(kenny_dump_flag == 1)
	    	{
	    cout << sample[i].getVector()[0] << ", " 
		 << sample[i].getVector()[1] << ", " 
		 << sample[i].getVector()[2] << ", " 
		 << sample[i].getVector()[3] << endl;
	    	}
	    */	
		//end added
    ...
    QuadFloat coord, f32bit parameter)
{
    ...
    if (textQueue[freeTexture[firstFree]].requested == 0)
    {
        ...
        //textQueue[freeTexture[firstFree]].textUnit = shInstr->getOp2(); //kenny disable this and replace by R15
	
	//kenny add to update using R15  information inside sampler
	//cout << "kenny sampler:(f32bit)" << R15[0] << " vs (u32bit)" << (u32bit)R15[0] << endl;
	//cout << "kenny sampler: " << R15_0_uint << endl;
	textQueue[freeTexture[firstFree]].textUnit = R15_0_uint;
	// end add to R15
    ...
    else
    {
        ...
        GPU_ASSERT(
		   /* kenny disable to suppress error when switch to use R15
		   ...
		   //kenny add to replace the panic using new R15.		   
		   if (textQueue[freeTexture[firstFree]].textUnit != R15_0_uint)
                panic("ShaderEmulator", "textureOperation", "Accessing different texture units from the same stamp. Might be the casting problem when adapting f32bit into u32bit.");
...
bool ShaderEmulator::checkJump(gpu3d::ShaderInstruction::ShaderInstructionDecoded *shInstrDec, u32bit vectorLength, u32bit &destPC)
{
	...
	// kenny: need to check to support RET instruction for jumping to absolute address store in register
	// if (unsigned int IMM & 3221225472 == 1073741824)
	//{ int R = IMM & 31
	//  destPC=getvalof(R.x);
	//}
	unsigned int IMM;
	IMM = shInstrDec->getShaderInstruction()->getJumpOffset();
	//cout << IMM << endl;
	if (IMM >= 1000000)
	  {
	    IMM -= 1000000;  //find the target register that contain jump offset

	    cout << "jumping to register: r" << ((f32bit *)this->temporaryBank)[(4 * numTemporaryRegs + IMM) * (UNIFIED_TEMP_REG_SIZE/4) + 0] << endl;
	  }  
    }
    else
    {
...
//kenny overloading the checkJump to only check if this is a just instruction but now doing the jumping
bool ShaderEmulator::checkJump(gpu3d::ShaderInstruction::ShaderInstructionDecoded *shInstrDec)
{   

    bool jump;
    
    std::cout << "change me!! Im in ShaderEmulator.cpp checkJump overloading\n";
...
void ShaderEmulator::shMOV(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
    ...
    //#ifdef KENNY_DBG
    //kenny debug add for counting vertice number
    shInstr.getShaderInstruction()->disassemble(disInstr);
    //cout << "kenny::disInstr: " << disInstr << endl;
    if(!strcmp("mov r33, i1", disInstr))
      {
	vertice_count++;
	//cout << "Thread[" << shInstr.getNumThread() << "] processing " << "Vertice number: " << vertice_count << endl;
      }
    //#endif
    ...
    //kenny dump attribute.
    /*
    if (!strcmp("mov r32, r32", disInstr))
      {
	cout << op1[0] << " "
	     << op1[1] << " "
	     << op1[2] << " "
    ...
    */
    ...
    //kenny debug dumping function dumpifIwanna()
#ifdef DEBUGforREGdump
    shEmul.dumpifIwanna(shEmul);
#endif
}
...
void ShaderEmulator::shTEX(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
    ...
    //kenny change to support our texture loading instruction format
    // NSYSU GLSL compiler use tex r16, r16, r15 which the op2(r15) stores
    // the texture unit used by the shader code.
    shEmul.read2Operands(shInstr, shEmul, op1, op2);
    R15_0_uint = (int)floor(op2[0]+0.5);
	if(R15_0_uint==0||R15_0_uint==0)
		kenny_dump_flag = 1;
	else
		kenny_dump_flag = 0;
...
//kenny implement GLSL built-in function: discard() using shKIL
extern bool k_jump[4];
void ShaderEmulator::shKIL(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
    ...
    if ((op1[0] < 0.0f) || (op1[1] < 0.0f) || (op1[2] < 0.0f) || (op1[3] < 0.0f))
    {
      ...
      // kenny add an if statment to use the previous SETPGT prediction register as a flag to identify which
      // pixel in the QUAD is actually been KIL. ( for here, it is the one that are not JMP )
      if(k_pred_reg[shInstr.getNumThread()] == false)
      {  
...
//PRINT_OPERATION_RES_3
    ...
    {
    */
    if (result[0] < 0.0 || result[1] < 0.0 || result[1] < 0.0 || result[1] < 0.0)
      {
        /*  Set the thread kill flag.  */
        shEmul.kill[shInstr.getNumThread()][shEmul.sampleIdx[shInstr.getNumThread()]] = TRUE;
      }
...
//kenny add counter for %4
int setpgt_counter = 0;
void ShaderEmulator::shSETPGT(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
    ...
    //kenny add to dump op1
    /*    
    cout << op1 << " ";
    if(setpgt_counter%4==3)
      cout << endl;
    setpgt_counter++;
    */
    //result=(setpgt_counter++)&1;

    //  Write instruction result.
    shEmul.writeResult(shInstr, shEmul, result);
    k_pred_reg[shInstr.getNumThread()] = result;
    //cout << k_pred_reg[shInstr.getNumThread()];
...
//Kenny add for LD/ST instruction (LDV & STV)

//Date 04/25 Two problems remain 1. I have to manually remove ExternalMemory.dat everytime I execute it... 2. the Float Interger problem
// 05/11 Both problems are solved.

//LDV Res, op1, op2 (ATTILA)
//LDV R1, R2, IMM (NSYSU)
//R1.xyzw=MEM[R2.x+Imm].xyzw

void ShaderEmulator::shLDV(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
    QuadFloat op1;
    QuadFloat op2;
    f32bit result[4];
    //ifstream iFile;
    //char fileString[100];
    //ExternalMemory* m_ptr;
    u32bit address;
    u32bit threadNum;
    //iFile.open("ExternalMemory.dat", ios::in);
    //if(!iFile.is_open())
    //panic("ShaderEmulator", "ShaderEmulator", "Error opening ExternalMemory.dat");
    ...
    //  Perform instruction operation.
    //GPUMath::ADDI(op1.getVector(), op2.getVector(), result);
    //search external memory for address op1[0] + op2[0] and return the value to result.
    address = floor(op1[0] + op2[0] + 0.5);
    threadNum = shInstr.getNumThread();
    //cout << "Loading address: op1[0] + op2[0] = " << op1[0] << " + " << op2[0] << " = " << address << endl;

    if(address < MAX_MEMORY_ADDRESS && address >= 0)
      {
	result[0] = ExternalMemory[threadNum][address][0];
	result[1] = ExternalMemory[threadNum][address][1];
	result[2] = ExternalMemory[threadNum][address][2];
	result[3] = ExternalMemory[threadNum][address][3];
	/*
	if(ExternalMemory[threadNum][address][4] == 0)
	  cout << "Warning::ShaderEmulator::Thread[" << threadNum <<"]PC[" << shEmul.PCTable[threadNum] << "]:shLDV loading uninitialized memory address: " << address << ", vertice_number: " << vertice_count <<endl;
	*/
      }
    else
      {
	cout << "ERROR: Memory access over MAX_MEMORY_ADDRESS: " << MAX_MEMORY_ADDRESS <<endl;
		/*
		cout << "Dump ExternalMemory:\n";
		for(m_ptr = &EM_firstEntry; m_ptr->nextEntry != NULL; m_ptr = m_ptr->nextEntry)
		  cout << m_ptr->address[threadNum] << ": " << m_ptr->data[threadNum][0] << " " << m_ptr->data[threadNum][1] << " " << m_ptr->data[threadNum][2] << " " << m_ptr->data[threadNum][3] << "\n";
		cout << m_ptr->address[threadNum] << ": " << m_ptr->data[threadNum][0] << " " << m_ptr->data[threadNum][1] << " " << m_ptr->data[threadNum][2] << " " << m_ptr->data[threadNum][3] << "\n\n";
		*/
		result[0] = 0;
		result[1] = 0;
...
//cout << "result[0]: " << result[0] << endl;
    ...
    //iFile.close();
    //cout << "LDV says: HELLO WORLD!\n";
    
}

//STV Res, op1, op2 (ATTILA)
//STV R1, R2, IMM (NSYSU)
//Mem([R1]+Imm).xyzw <- [R2].xyzw

void ShaderEmulator::shSTV(ShaderInstruction::ShaderInstructionDecoded &shInstr, ShaderEmulator &shEmul)
{
  static int shSTV_flag;
    f32bit *res;
    QuadFloat op1;
    QuadFloat op2;
    //ExternalMemory* m_ptr;
    ...
    //oFile.open("ExternalMemory.dat", ios::app);

    //  Read instruction operands.
    shEmul.read2Operands(shInstr, shEmul, op1, op2);
    res = (f32bit *) shInstr.getShEmulResult();

    //  Perform instruction operation.
    address = floor(res[0] + op2[0] + 0.5);
    threadNum = shInstr.getNumThread();
    //cout << "Storing address: res[0] + op2[0]: " << res[0] << " + " << op2[0] << endl;
    //cout << "Store to address: " << address << endl;
    //cin >> address;

    //  Write instruction result.
    if(address < MAX_MEMORY_ADDRESS && address >= 0)
      {
	ExternalMemory[threadNum][address][0] = op1[0];
	ExternalMemory[threadNum][address][1] = op1[1];
	ExternalMemory[threadNum][address][2] = op1[2];
	ExternalMemory[threadNum][address][3] = op1[3];
	ExternalMemory[threadNum][address][4] = 1;
      }
    else
      {
	cout << "ERROR: Memory access over MAX_MEMORY_ADDRESS: " << MAX_MEMORY_ADDRESS <<endl;
    ...
    /*
    if(threadNum == 0 && address <= 17 && shSTV_flag == 0)
      {
	cout << address << ": ";
	cout << ExternalMemory[threadNum][address][0] << ", "
	     << ExternalMemory[threadNum][address][1] << ", "
	     << ExternalMemory[threadNum][address][2] << ", "
	     << ExternalMemory[threadNum][address][3] << endl;
      }
    else
      shSTV_flag = 1;
    */
    //oFile << res[0] + op2[0] << ": " << op1[0] << " " << op1[1] << " " << op1[2] << " " << op1[3] << endl;
    ...
    cout << "Dump ExternalMemory:\n";
    for(m_ptr = &EM_firstEntry; m_ptr->nextEntry != NULL; m_ptr = m_ptr->nextEntry)
      cout << m_ptr->address << ": " << m_ptr->data[0] << " " << m_ptr->data[1] << " " << m_ptr->data[2] << " " << m_ptr->data[3] << "\n";
      cout << m_ptr->address << ": " << m_ptr->data[0] << " " << m_ptr->data[1] << " " << m_ptr->data[2] << " " << m_ptr->data[3] << "\n\n";
      }
    */
}
...
void (*ShaderEmulator::shInstrEmulationTable[])(ShaderInstruction::ShaderInstructionDecoded &, ShaderEmulator &) =
{
    ...
    shLDV,    shSTV                             //  Opcodes 38h - 39h Kenny add for LD/ST
};
