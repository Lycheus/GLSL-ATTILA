...
ShOpcode gpu3d::translateShOpcodeTable[] =
{
    ...
    LDV,    STV,    INVOPC, INVOPC, INVOPC, INVOPC, INVOPC, INVOPC      //  Opcodes 38h - 3Fh
};
...
ShaderInstruction::ShaderInstruction(u8bit *instrCode)
{
    ...
    isIntegerB = (opcode == ARL) || (opcode == ADDI) || (opcode == MULI) || (opcode == STPEQI) || (opcode == STPGTI) || (opcode == STPLTI);//|| (opcode == LDV) || (opcode == STV);
        ...
        default:            
            ...
            {        
                ...
                if (isALoadB)
                {
                    ...
                    //bankOp2 = TEXT; // kenny disable
		    // kenny change to make texture bankOp2 is TEMP BANK
		  bankOp2 = TEMP;
                }
    ...
    endFlag(lastInstr), waitPointFlag(waitPoint)
{
    ...
    switch(opcode)
    {
        ...
        default:
		    ...
		    //bankOp2 = TEXT; //kenny disable
		    //kenny change bankOp2 to TEMP BANK for NSYSU GLSL compiler
		  bankOp2 = TEMP;
...
ShaderInstruction *ShaderInstruction::assemble(char *line, string &errorString)
{
    ...
    //  Set if this instruction is an integer instruction.
    bool integerOp = (opc == ARL) || (opc == ADDI) || (opc == MULI) || (opc == STPEQI) || (opc == STPGTI) || (opc == STPLTI); //|| (opc == LDV) || (opc == STV);
    ...
    //Kenny Add for debug assembler
    //cout << "KENNY::" << line << " -- line[" << i << "]:" << line[i] << " length: " << length << endl;
            CHECK_PARSING_ERROR(i != length, "missing instruction parameters")
    ...
    bool &relMode, u32bit &relModeR, u8bit &relModeComp, s16bit &relModeOff)
{
    ...
    //Kenny Add for debug assembler
    //cout << "KENNY::" << line << " -- line[" << i << "]:" << line[i] << " regStr: " << regStr << endl;
...
u32bit ShaderInstruction::setNumOperands(ShOpcode opcode)
{
    ...
    switch(opcode)
    {
        ...
        case MULI: case STPEQI: case STPGTI: case STPLTI: case LDV: case STV:
...
bool ShaderInstruction::setIsSOACompatible(ShOpcode opcode, MaskMode writeMask)
{
    switch(opcode)
    {
        ...
        case ADDI: case MULI: case LDV: case STV:
