...
bool ShaderOptimization::isVectorOperation(ShOpcode opc)
{
    return (opc == ADD)    || (opc == CMP) || (opc == CMPKIL)  || (opc == MAD) || (opc == FXMAD) ||
           ...
           (opc == ADDI) || (opc == MULI) || (opc == LDV) || (opc == STV);
}
    ...
    for(u32bit instr = 0; instr < inProgram.size(); instr++)
    {
        ...
        switch(inProgram[instr]->getOpcode())
        {
            ...
            default:
			    ...
			    //Kenny debug
			    // If optimization is enable, they test temp register if it is initial or not. This makes problem.
			    // Because I use all r0-99 freely when converting NSYSU GPU assembly into ATTILA assembly.
			    // If anyone enable the optimization, please deal with this problem.
			    //Kenny end debug
                        }
                    }
                }
