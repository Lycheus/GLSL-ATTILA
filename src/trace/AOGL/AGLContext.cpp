...
#include <stdio.h>
#include <stdlib.h>
int kenny_temp = 0; //kenny add for presentation at NCTU 1/19
//end of Kenny include
...
extern uint current_shader_program_index;  //kenny add to support glUseProgram
extern list<uint> program_shader_list[1024]; //kenny add to support glAttachProgram
...
void GLContext::setShaders()
{
	...
	{
	  // kenny add for presentation at NCTU 1/19
	  /*
	  if(kenny_temp == 0){
	  cout << "Start to compile GLSL vertex shader code for the program (press enter to continue...)\n";
	  cin.ignore();
	  system("cp ../120_morphing/vertex.bin ../120_morphing/fragment.bin ./");
	  }
	  */	 
	  //end kenny add
	  //kenny tag compile
		...
		//Kenny add for using hand-tweek binary
		FILE* vertex_bin_file;
		size_t file_size;
		acd_ubyte* buffer;
		static int kenny_print_bin_flag;
		char file_name[256];				
		sprintf(file_name, "./converter/%d.bin", *(program_shader_list[current_shader_program_index].begin()));
		//cout << "kenny::" << file_name << endl;
				
		vertex_bin_file = fopen(file_name, "r");
		if (vertex_bin_file!=NULL)
		  {
		    if(kenny_print_bin_flag == 0)
		      {
			std::cout << "NEW VERTEX.BIN FOUND, GOING TO USE IT!\n";
			kenny_print_bin_flag = 1;
		      }
		    fseek(vertex_bin_file , 0 , SEEK_END);
		    file_size = ftell(vertex_bin_file);
		    rewind(vertex_bin_file);
		    buffer = (acd_ubyte*)malloc(sizeof(acd_ubyte)*file_size);
		    fread(buffer, sizeof(acd_ubyte), file_size, vertex_bin_file);
		    vsh->setCode(buffer, file_size);
		    fclose(vertex_bin_file);
		  }
		else
		  std::cout << endl << "No " << file_name << " vertex shader to read, using the default binary\n";
		
		//Kenny add for dump attilaASM VS
		/*
		if(current_shader_program_index == 54)
		  {
		std::cout << "===============VS================\n";
		std::cout << "print vertex shader Constants\n";
		vsh->printConstants(cout);
		//std::cout << "print vertex shader attilaASM\n";		
		//vsh->printASM(cout);
		std::cout << "===============VS================\n\n";
		  }
		*/
		...
		//kenny add for demo to NCTU 1/19
		/*
		  if(kenny_temp == 0){
		  system("cat ./minimal.vert");
		  cout << "Reading the GLSL source code" << endl;
		  cin.ignore();
		  }
		*/
		//end add			        
		...
		cout << "\nCompiled GLSL shader code for the program (press enter to continue...)\n";
		cin.ignore();

		system("cat ./attilaASM.vert");
		cout << "\nConverted into attilaASM (press enter to continue...)\n";
		cin.ignore();
		}
    ...
    if ( _renderStates[RS_FRAGMENT_PROGRAM] )
	{
		...
		sprintf(file_name, "./converter/%d.bin", *(++(program_shader_list[current_shader_program_index].begin())));
		//cout << "kenny::" << file_name << endl;
		fragment_bin_file = fopen(file_name, "r");
		if (fragment_bin_file!=NULL)
		  {
		    if(kenny_print_bin_flag == 1)
		      {
			std::cout << "NEW FRAGMENT.BIN FOUND, GOING TO USE IT!\n";
			kenny_print_bin_flag = 2;
		      }
		    fseek(fragment_bin_file , 0 , SEEK_END);
		    file_size = ftell(fragment_bin_file);
		    rewind(fragment_bin_file);
		    buffer = (acd_ubyte*)malloc(sizeof(acd_ubyte)*file_size);
		    fread(buffer, sizeof(acd_ubyte), file_size, fragment_bin_file);
		    fsh->setCode(buffer, file_size);
		    fclose(fragment_bin_file);
		  }
		else
		  std::cout << "No " << file_name << " fragment shader to read, using the default binary\n";
		//end add

		//Kenny add for dump attilaASM FS
		/*
		if(current_shader_program_index == 54)
		  {		
		    std::cout << "===============FS================\n";
		    std::cout << "print fragment shader Constants\n";
		    fsh->printConstants(cout);
		    //std::cout << "print fragment shader attilaASM\n";
		    //fsh->printASM(cout);
		    std::cout << "===============FS================\n\n";
		  }
		*/
