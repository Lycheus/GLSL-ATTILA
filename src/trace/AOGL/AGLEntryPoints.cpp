...
//kenny add to support uniform
GLAPI void GLAPIENTRY AGL_glUniform1f( GLint location, GLfloat v0 )
{
  return;
}
GLAPI void GLAPIENTRY AGL_glUniform1i( GLint location, GLint v0 )
{
  return;
}
GLAPI void GLAPIENTRY AGL_glUniform2f( GLint location, GLfloat v0, GLfloat v1 )
{
  return;
}
GLAPI void GLAPIENTRY AGL_glUniform3fv( GLint location, GLsizei count, const GLfloat *value )
{
  return;
}
GLAPI void GLAPIENTRY AGL_glUniformMatrix3fv( GLint location, GLsizei count, GLboolean transpose, const GLfloat *value )
{
  cout << value[0];
  return;
}
GLAPI void GLAPIENTRY AGL_glUniformMatrix4fv( GLint location, GLsizei count, GLboolean transpose, const GLfloat *value )
{
  cout << value[0] << " " << value[15] << endl;
  return;
}


// Kenny add to support glUseProgram
std::list<uint> program_shader_list[1024];
uint current_shader_program_index;

GLAPI void GLAPIENTRY AGL_glUseProgram( GLuint index )
{
  current_shader_program_index = index;
}
// end of glUseProgram

//kenny add to support glAttachShader
GLAPI void GLAPIENTRY AGL_glAttachShader( GLuint index, GLuint shaderID )
{
  program_shader_list[index].push_back(shaderID);
}
...
//kenny add to support glBindFramebuffer
GLAPI void GLAPIENTRY AGL_glBindFramebuffer( GLenum target, GLuint index )
{
}
...
GLAPI void GLAPIENTRY AGL_glEnable( GLenum cap )
{
    ...
    switch ( cap ) {
		...
		//kenny debug add
		//std::cout << "glEnable(GL_TECTURE_2D) activated\n";
            }
            break;
        case GL_TEXTURE_GEN_S:
        case GL_TEXTURE_GEN_T:
...
GLAPI void GLAPIENTRY AGL_glEnd( void )
{
    ...
    //Kenny add
    //cout << "\nglEnd detected and execute setShaders next line\n";
    //add end
    _ctx->setShaders();
...
{ 
  if (len == -1)
      len=(GLsizei)strlen((const char*)str);
...
GLAPI void GLAPIENTRY AGL_glDrawElements( GLenum mode, GLsizei count, GLenum type, const GLvoid *indices )
{
    ...
    //if (current_shader_program_index == 54) //kenny render specific shader program item
    //{
    ...
    cout << "Program("<< current_shader_program_index << ") ";
    cout << "drawing:" << count << endl;
    //kenny dump vertex data in glDrawelements
    //acd_ubyte* data = indicesBuffer.getData();
    //}
...
{
  //kenny add to display more detailed DrawArrays information for debug purpose
  cout << "glDrawArray::DRAWING " << count << " VERTEX\n";
  
    GLOBALPROFILER_ENTERREGION("glDraw", "", "")

    AGL_PRINTCALL_3(mode,first,count);

    //kenny add for vertice count in every glDrawArrays (combo with ShaderEmulator.cpp to display the current vertice number for debug purpose)
    vertice_count=0;
...
//kenny debug accdump.txt acdxjustbeforedraw.txt
//#define ACD_STATS 1
#ifdef ACD_STATS
    _ctx->acd().DBG_dump("acddump.txt", 0);
    _ctx->fixedPipelineState().DBG_dump("acdxjustbeforedraw.txt",0);
                                        ...
                                        const GLvoid *pixels )
{
    ...
    */
    ...
    //kenny add to support type GL_UNSIGNED_SHORT_5_6_5
    //I didn't convert 5_6_5 in AGLTextureObject.cpp convertRGBtoRGBA
    //because there are difficulties adapting getSize() without crashing other funcs
    if(type == GL_UNSIGNED_SHORT_5_6_5 && pixels != NULL)
      {
	//casting data from 5_6_5 to UNSIGNED_BYTE
	cout << "KENNY: converting 5_6_5 into unsigned byte\n";
	type = GL_UNSIGNED_BYTE;
	format = GL_RGBA;

	const GLushort* us_iptr = (GLushort*)pixels; //input
	GLubyte* rgba = new GLubyte[width*height*4]; //output
	GLubyte* ub_optr = rgba;
	GLsizei texels = width * height;
	GLdouble r, g, b;

	for( GLint i = 0; i < texels; i++)
	  {
	    r = GLdouble((us_iptr[i] & 0xF800) >> 11) / 0xFF;                          
	    g = GLdouble((us_iptr[i] & 0x07E0) >> 5) / 0xFF;                           
	    b = GLdouble((us_iptr[i] & 0x001F)) / 0xFF;    
	...
	to.setContents(target, level, internalFormat, width, height, 1, border, format, type, (const GLubyte*)rgba);
	return;
      }
    //end of 5_6_5 conversion

    to.setContents(target, level, internalFormat, width, height, 1, border, format, type, (const GLubyte*)pixels);
    //cout << "glTexImage2D::setContents END\n\n";

    //kenny add static glTexImage3D counter
    /*
    static int kenny_TexI2D_count=0;
    kenny_TexI2D_count++;

    if(pixels != NULL)
      printf("counter: %d\n", kenny_TexI2D_count);

    //if (kenny_TexI2D_count >= 1 && kenny_TexI2D_count < 100)
    if (kenny_TexI2D_count == 77)
      { 
	float *z=(float*)pixels;

	if(pixels == NULL)
	  printf("pixels pointer is NULL\n");
	else{
	FILE *f;
	f = fopen("TexImage2D_dump", "w");
	if (f != NULL)
	  { int i,l;
		...
		for(l=0;l<4;l++)
		  {		    
		    //printf(" %f", *(pixels+i*4+l));
		    printf(" %f", z[i*4+l]);
		  }
		printf("\n");
	      }
	    fwrite(pixels, sizeof(GLfloat), width*height*4, f);
	    fclose(f);
	  }
	else
	  printf("glTexImage2D::CANNOT OPEN FILE\n");
	}
      }
    */
    GLOBALPROFILER_EXITREGION()
                                                ...
                                                GLboolean normalized, GLsizei stride, const GLvoid *pointer)
{
    ...
    //Kenny marked
    //printf("Kenny@AGLEntryPoints:L1813 -> dumping VArray\n");
    //_ctx->getGenericVArray(index).dump();
    //Kenny dump
...
//Kenny modify glProgramEnvParameter4fvARB
GLAPI void GLAPIENTRY AGL_glProgramEnvParameter4fvARB (GLenum target, GLuint index, const GLfloat *params)
//GLAPI void GLAPIENTRY AGL_glProgramEnvParameter4fvARB (GLenum target, GLuint index, const GLfloat x, GLfloat y, GLfloat z, GLfloat w) // this is new but fail
{
    GLOBALPROFILER_ENTERREGION("AOGL", "", "")
    ...
    //Kenny mod input parameter of glProgramEnvParameteARB
    env.set(index,params[0], params[1], params[2], params[3]);
    //env.set(index, x, y, z, w); // new but fail
