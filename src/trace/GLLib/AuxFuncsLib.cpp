                            ...
                            std::set<GLuint>& iSet)
{
	...
	//added by sheng-chih
	//I want to change the vertex data in this point
	/*
	if(attrib == GPUDriver::VS_VERTEX)
	  {
	    int count_vertex = 0;
	    GLfloat *temp_ptr_f = (GLfloat*)(auxData + va.offset);
	    
	    while(count_vertex<=2)
	      {
		...
		for(i=0;i<1;i++)
		  {
		    int j;
		    for(j=0;j<va.components;j++)
		      {
			printf("%f",temp_ptr_f[j]);
			if(j<va.components-1)
			  printf(",");
		      }
		    printf("}\n");
		  }
		...
		if(count_vertex == 2)
		  temp_f = 5.0;
		memcpy(&temp_ptr_f[0],&temp_f,4);
		memcpy(&temp_ptr_f[1],&temp_f,4);
		memcpy(&temp_ptr_f[2],&temp_f,4);
...
void afl::setupShaders(GLContext* ctx, ::GPUDriver* driver)
{
    ...
    //kenny add for dump VS ASM
    //cout << "dumping VS ASM:\n";
    //vp->printASM();
    //end dump ASM

    //kenny add for dump FS ASM
    //cout << "dumping FS ASM:\n";
    //fp->printASM();
    //end dump ASM
