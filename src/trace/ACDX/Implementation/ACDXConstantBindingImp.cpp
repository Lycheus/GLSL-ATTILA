...
void ACDXConstantBindingImp::resolveConstant(const ACDXFixedPipelineState* fpState, ACDXFloatVector4& constant) const
{
    ...
    //kenny print resolving constant for debug
    //cout << "resolving constant\n";
    ...
    while ( iter != _vStateIds.end() )
    {
        ...
        switch ((*iter))
        {
		  ...
		  //kenny add for transpose Matrix (here is not where modeview & projection matrix are touched
		  cout << "kenny: ismatrix" << endl;
		  //cout << "isMatrix: " << isMatrix << "\nmId: " << mId << "\ntype: " << type << endl;
