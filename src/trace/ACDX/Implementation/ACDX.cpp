...
void acdlib::ACDXGenerateVertexProgram(ACDXFixedPipelineState* fpState, const ACDX_FIXED_PIPELINE_SETTINGS& fpSettings, ACDShaderProgram* &vertexProgram)
{
	...
	//kenny
	/*
	cout << "ACDXGenerateVertexPrograms" << endl;
	cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB\n";
	cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB\n";
	*/
...
void acdlib::ACDXGeneratePrograms(ACDXFixedPipelineState* fpState, const ACDX_FIXED_PIPELINE_SETTINGS& fpSettings, ACDShaderProgram* &vertexProgram, ACDShaderProgram* &fragmentProgram)
{
	...
	//kenny
	/*
	cout << "ACDXGeneratePrograms" << endl;
	cout << "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n";
	cout << "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n";
	*/
...
ACDXCompiledProgram* acdlib::ACDXCompileProgram(const std::string& code)
{
    ...
    u32bit size_binary;
    // kenny modified MAX_REG_NUM
    ACDXRBank<float> clusterBank(1000,"clusterBank");
...
ACDXConstantBindingList* acdlib::ACDXResolveProgram(ACDXFixedPipelineState* fpState, const ACDXCompiledProgram* cProgram, const ACDXConstantBindingList* constantList, ACDShaderProgram* program)
{
    ...
    //cout << "kenny: print blusterBank before merged with GLState constants\n";
    //clusterBank.print(cout); //kenny print clusterBank before merge with GLStat constants
    ...
    // kenny transpose all Modelview & Projection Matrix for NSYSU LLVM GLSL compiler
    // WARNING: THIS CODE WILL NULLIFIED ALL ATTILA MODELVIEW & PROJECTION MATRIX TO TRANSPOSED M    
    fpStateImp->transposeProjectionMatrix();
    fpStateImp->transposeModelviewMatrix(acd_uint(0));
    // end kenny modified

    // Synchronize ACDXFixedPipelineState states to ACDXGLState
    fpStateImp->sync();

    //fpStateImp->DBG_dump("dump_state", 0); //kenny gone craZy! HAHAHA
	...
	//kenny print constant
	//cout << constant[0] << " " << constant[1] << " " << constant[2] << " " <<  constant[3] << endl;
    ...
    //kenny test print shader constants
    //cout << "kenny print program Constants:\n";
    //program->printConstants(cout);
