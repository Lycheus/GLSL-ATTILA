...
void ACDXFixedPipelineStateImp::setModelviewMatrix(acd_uint unit, const ACDXFloatMatrix4x4& matrix)
{
    ...
    //cout << "KENNY: going to change here @ACDXFixedPi... for model view transpose\n";
    //kenny : you cant change here to achieve transpose, every fixed pipeline matrix access will touch this function.
...
//kenny add transposeModelviewMatrix
void ACDXFixedPipelineStateImp::transposeModelviewMatrix(acd_uint uint)
{
  ACDXFloatMatrix4x4 matrix = getModelviewMatrix(acd_uint(0));
  ACDXFloatMatrix4x4 trans_m(acd_float(0));
  ...
  setModelviewMatrix(acd_uint(0), trans_m);
  /*
  ACDXFloatMatrix4x4 new_modelview_matrix = getModelviewMatrix(uint);
  cout << "kenny: modelview matrix before transpose\n";
  for (int i=0; i<16; ++i)
    cout << new_modelview_matrix[i] << " ";
  cout << endl;
  
  //new_modelview_matrix.transpose();
  
  cout << "kenny: after transpose\n";
  for (int i=0; i<16; ++i)
    cout << new_modelview_matrix[i] << " ";
  cout << endl;
  
  setModelviewMatrix(0, new_modelview_matrix.transpose());
  */
}
//end kenny add
...
{
    //cout << "KENNY: going to change here @ACDXFixedPi... for projection transpose\n";
    //kenny : you cant change here to achieve transpose, every fixed pipeline matrix access will youch this function.
  //cout << "Projection Matrix:\n";
  //matrix.print(cout);
  //cout << endl;
  _projectionMatrix = matrix;
  //ACDXFloatMatrix4x4 ret(acd_float(0));
  //_projectionMatrix = ret;
}
...
//kenny add transposeProjectionMatrix
void ACDXFixedPipelineStateImp::transposeProjectionMatrix()
{
  ACDXFloatMatrix4x4 matrix = getProjectionMatrix();
  ACDXFloatMatrix4x4 trans_m(acd_float(0));
  for(int i=0; i<4; i++)
    {
      for(int j=0; j<4; j++)
...
{
  // kenny force sync all the Fixed Pipeline State
  //_requiredSync = 1;
  //cout << "kenny force sync\n";
  // end kenny add
