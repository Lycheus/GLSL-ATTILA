...
#ifndef ALREADYDEFINEDcntttt
extern int cntttt;
extern bool r50nr11diff;
#endif

namespace gpu3d
{

  //kenny add a printfspam to skip irrelevant data output
  //#define printfspam(x) {if (cntttt > 1900000) printf("made it to %s\n",x);}    
//#define printfspam(x) {if (cntttt > 1900628) printf("made it to %s\n",x);}    
#define printfspam(x) {;}    
//int cntttt = 0;
...
//kenny change STAMP_FRAGMENTS from 4 to 1
//fail because TextureEmulator only supported stamps with 4
//static const u32bit STAMP_FRAGMENTS = 1;
//static const u32bit STAMP_WIDTH = 1;
//static const u32bit STAMP_HEIGHT = 1;
...
static const u32bit UNIFIED_INSTRUCTION_MEMORY_SIZE = 4096;
...
//static const u32bit UNIFIED_TEMPORARY_NUM_REGS = 32; //kenny modified temp register number
static const u32bit UNIFIED_TEMPORARY_NUM_REGS = 64;
